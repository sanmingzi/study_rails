...
N + 1 问题是数据库访问中最常见的一个性能问题。举个例子，在一个权限数据库中有三张表，roles / permissions / role_permissions。想要得到所有的 role 以及其对应的 permission，一种写法是：
...
# We can use the includes to fix it, the next code only access db one time.
